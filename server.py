        ...
        for emoji_file in os.listdir(emoji_dir):
            ...
            try:
                with Image.open(emoji_path) as emoji_image:
                    ...
                    emoji_colors.append(avg_color)  # Appending to the list before converting to numpy array
                    logging.debug(f'Loaded emoji {emoji_file} with average color {avg_color}')
            except Exception as e:
                logging.error(f'Error loading emoji {emoji_file}: {str(e)}')
                continue  # Skip this emoji but continue loading others
        # Convert emoji_colors to numpy array after appending all colors
...
# Call load_emoji_data() to initialize
load_emoji_data()
        ...
        for y in range(height):
            for x in range(width):
                ...
                try:
                    ...
                    continue  # Skip this pixel but continue creating the mosaic
